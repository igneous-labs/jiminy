...
///
/// To access the underlying account, you must use this handle with [`Abr::get`] or
/// [`Abr::get_mut`].
///
/// # Implementation details
///
/// - the contained pointer points to the start of account data; the 88-bytes before it
///   is the [`Account`]  
/// - the `'account` lifetime spans from when the [`Accounts`] struct is deserialized at the start
///   of the program to when the same `Accounts` struct is dropped on program exit. This is usually `'static`.
///
/// # Note on core traits
///
...
impl Ord for AccountHandle<'_> {
    ...
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        self.account_data.cmp(&other.account_data)
    }
}
